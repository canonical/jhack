...
from jhack.helpers import current_model, list_models
...
def _exec(cmd):
    ...
    if return_code != 0:
        raise RuntimeError(f"{cmd!r} failed with retcode {return_code!r}; ")
    return return_code
...
async def _remove_model(
    ...
    cmd = (
        f"juju destroy-model {model_name} "
    ...
    print(
        f'{"nuking" if nuking else "shutting down"} :: {model_name} {"⚛" if nuking else "✞"}'
    )
