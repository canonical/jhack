...
def unbork_juju(
    ...
    if not unbork_juju_script.exists():
        raise RuntimeError(
            f"unbork_juju script not found. Is it where it should be? {unbork_juju_script}"
        )
    ...
    if not unbork_juju_script.exists():
        raise RuntimeError(f"unable to locate unbork_juju shell script ({unbork_juju_script!r})")
