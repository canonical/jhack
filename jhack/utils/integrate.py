...
from typing import TypedDict, Dict, List, Tuple, Set, Sequence, Optional
...
from jhack.helpers import juju_status, fetch_file, JPopen, ColorOption, RichSupportedColorOptions
...
def _gather_endpoints(model=None, apps=()) -> Dict[AppName, _AppEndpoints]:
    ...
    for app_name, app in status['applications'].items():
        ...
        for role in ('requires', 'provides'):
            role_eps = {ep: (spec['interface'], remotes(app, ep)) for ep, spec in meta.get(role, {}).items()}
...
class IntegrationMatrix:
    def __init__(self, apps: str = None, model: str = None,
    ...
    def _build_matrix(self):
        ...
        for provider, requirer in self._pairs():
            ...
            shared = sorted(set(intf[0] for intf in provides.values()
                                ).intersection(
    ...
    def watch(self, refresh_rate=.2):
        rrate = refresh_rate or .2
        live = Live(get_renderable=partial(self.render, refresh=True),
                    refresh_per_second=rrate)
        live.start()
    ...
    def _apply_to_all(self, include: str, exclude: str, verb: str,
                      juju_cmd: str, dry_run: bool = False, active: bool = None):
        targets = self._apps
            ...
            for interface in interfaces:
                if active in {True, False}:
                    ...
                    if self._is_active(interface, prov, req) is not active:
                        logger.debug(f'skipping {prov}:({interface}) <--> {req}: '
    ...
    def connect(self, include: str = None, exclude: str = None, dry_run: bool = False):
        self._apply_to_all(include, exclude, verb='connect', juju_cmd='relate',
                           dry_run=dry_run, active=False)

    def disconnect(self, include: str = None, exclude: str = None, dry_run: bool = False):
        self._apply_to_all(include, exclude, verb='disconnect', juju_cmd='remove-relation',
                           dry_run=dry_run, active=True)
...
def show(
        apps: str = typer.Argument(
            None,
            help="Regex to filter the applications to include in the listing."),
        watch: bool = typer.Option(
            None, '--watch', '-w',
            help="Keep this alive and refresh"),
        refresh_rate: float = typer.Option(
            None, '--refresh-rate', '-r',
            help="Refresh rate for watch."),
        model: str = typer.Option(
            None, '--model', '-m',
            help="Model in which to apply this command."),
