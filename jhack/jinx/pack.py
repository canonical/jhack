...
def pack(
    charm_source: Path = Argument(
        None,
        help="Path to file containing the Jinx subclass to pack; "
    ...
    charm_source = (
        Path(charm_source) if charm_source else Path() / "src" / "charm.py"
    ).absolute()
    ...
    if cmd.returncode != 0:
        print(
            "command exited with code nonzero: something went wrong. "
